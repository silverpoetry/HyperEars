...
- 新增 Edifier W820NB 双金标版实机适配。该型号使用 V1（bleVersion=1）BES 帧格式
  （无加密，CRC16 以 8217 为种子），与家族其他型号的 BES v2 帧格式不同；识别后通过
  `0xD0` 提供私有整机电量，通过 `0xCC`/`0xC1` 提供降噪、关闭、通透三态控制
  （方言 `1=标准 2=降噪 3=通透`，通透档位第二字节暂不投影）。协议依据 Edifier Connect
  v8.4.48 实机 RFCOMM 抓包验证。
- W820NB 双金标版新增游戏模式：`0x08` 查询、`0x09` 开关，首次合法状态响应后开放，
  MiLink 卡片提供“游戏模式”开关；卡片同时按模块已确认状态高亮原生三态按钮的当前模式
  （原生卡片对私有通道设备不显示当前模式，由模块状态投影补齐）。
