...
- 该型号使用 2026 代环境声方言：状态以子类型 `0x19` 的通知帧推送（`0x15` 查询不回复），
  解析开关与模式字节后开放降噪、关闭、通透；写入帧按通知帧镜像。
- 解析与电量已依据真实设备日志验证；模式切换写入实机确认生效。

### Sony WH-1000XM4

- 修复 WH-1000XM4 私有通道握手：该固件忽略第一帧初始化请求，仅在重发后返回初始化响应
  （与 Sound Connect 抓包行为逐字节一致）；握手完成前收到任何设备帧都会重发 `00 00`。
- 适配走 v1 服务：整机电量、降噪/关闭/环境声/抗风噪三态与方言均按实机流量验证。

### 诊断

- 握手等待上限从 2.5 秒放宽至 6 秒，为慢启动的 v1 固件留出重发窗口。
- 传输失败日志补充具体原因（异常消息），便于定位 SDP/连接失败。
- 厂商控制 App 进程新增被动套接字监视（仅详细日志开启时记录），用于恢复官方 App 的
  私有协议握手；不修改 App 行为。
