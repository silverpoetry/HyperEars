...
- 新增 HUAWEI FreeClip 2 实机适配。该型号为开放式夹耳，无降噪与通透；只在收到合法
  电量帧后开放左右耳与充电盒组件电量，不发送 `2B 2A` 噪声查询，也不展示噪声控制。
- 兼容该型号实测的未分类蓝牙 Class `0x001F00`，使用完整的 `freeclip` 产品名称作为
  保守耳机准入提示；原版 FreeClip 与 FreeLace 继续使用既有家族和标准身份路径。

### Sony WF-1000XM6

- 新增 Sony WF-1000XM6 具体型号适配：v2 服务优先，握手、双耳与充电盒电量、三态降噪控制。
- 该型号使用 2026 代环境声方言：状态以子类型 `0x19` 的通知帧推送（`0x15` 查询不回复），
  解析开关与模式字节后开放降噪、关闭、通透；写入帧第三字节为切换确认提示音标志
  （与 Sound Connect 抓包一致，切换时播放耳机提示音）。
- 解析与电量已依据真实设备日志验证；模式切换与提示音实机确认生效。

### Sony WH-1000XM4

- 修复 WH-1000XM4 私有通道握手：该固件忽略第一帧初始化请求，仅在重发后返回初始化响应
  （与 Sound Connect 抓包行为逐字节一致）；仅该精确型号在握手前收到设备命令时允许重发
  一次 `00 00`，不会延长其他设备的握手时间。
- 适配走 v1 服务：整机电量、降噪/关闭/环境声/抗风噪三态与方言均按实机流量验证。
