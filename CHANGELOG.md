...
- 新增 HUAWEI FreeClip 与 FreeClip 2 适配。两型号为开放式夹耳，无降噪与通透，只在收到
  合法电量帧后开放左右耳与充电盒组件电量，不发送 `2B 2A` 噪声查询，也不展示噪声控制。
- 修正华为 FreeClip / FreeLace 产品线在设备蓝牙 Class 为未分类（`0x001F00`）时无法通过
  标准耳机识别门禁的问题，将 `freeclip` 与 `freelace` 加入保守名称标记。
