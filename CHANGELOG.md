...
- 新增贡献者实机验证的 FitBuds Turbo 明文 BES 协议适配，分别确认左右耳电量、四态降噪与
  游戏模式；规范化名称只选择候选，私有控制在有效回包后开放。
- 抗风噪与游戏开关复用共享原生卡片布局，分别按协议证据显示；会话重置清除游戏状态，
  模式写入后执行一次只读查询矫正，游戏切换以设备回报为准。

### 漫步者电量

- TWS 左右耳的零值统一显示为未知；双耳不可用的有效状态帧仍更新充电盒电量并清除旧耳电量。
  越界耳电量继续视为非法帧，整机与充电盒的真实 0% 保持有效。
