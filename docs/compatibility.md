...
| Edifier W820NB 双金标版 | 具体型号 | 实机验证（RFCOMM 抓包） | 规范化名称精确匹配；V1 协议合法电量与模式响应分别确认能力 | 系统整机 → 私有整机 | 降噪、关闭、通透 |
...
协议返回单一聚合值时镜像到左右耳，不把聚合值表述为独立组件遥测。W820NB 双金标版使用
V1（bleVersion=1）帧格式（无加密，`[header][len][cmd][payload][crc16]`，CRC16 以 8217 为
种子），ANC 方言为 `1=标准 2=降噪 3=通透`；其系统整机电量由 `0xD0` 单字节明文百分比
报告。游戏模式经 `0x08` 查询、`0x09` 开关，在合法状态响应后由 MiLink 卡片上的
“游戏模式”开关提供。
