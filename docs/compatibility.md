...
| Edifier FitClip Ultra | 具体型号 | 实机验证 | 规范化名称精确匹配；合法 `0xF2` 电量及 `0x08/0x09` 游戏状态分别确认能力 | 系统整机 → 私有组件 | 无；另支持游戏模式 |
| Edifier FitBuds Turbo | 具体型号 | 贡献者实机验证 | 规范化名称精确匹配；明文 BES 电量、`0x1B` 模式及游戏状态分别确认能力 | 系统整机 → 私有左右耳；盒字段按有效回包提供 | 降噪、关闭、通透、抗风噪；另支持游戏模式 |
...
FitBuds Turbo 的明文 BES 负载与模式控制依据 [PR #62](https://github.com/silverpoetry/HyperEars/pull/62)
的帧样例及贡献者实机结果；盒电量尚未单独实测。FitClip Ultra 与 Turbo 共用类型化游戏状态及
重置逻辑。Turbo 的降噪与游戏能力独立确认：仅游戏已确认时使用原生双选项卡，降噪已确认时使用
原生三态卡并按已确认状态分别展示抗风噪、游戏开关。维护版共享布局的实机复核结果另行跟踪。
