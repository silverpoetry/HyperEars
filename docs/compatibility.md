...
| HUAWEI FreeClip | 具体型号 | 参考协议 | 规范化型号精确匹配；Channel 1 合法电量帧确认私有电量 | 系统整机 → 私有组件 | 无（开放式夹耳，不开放降噪） |
...
RFCOMM Channel 16；FreeBuds Pro 3、FreeBuds 4、FreeClip 与 FreeClip 2 使用 Channel 1；
家族候选在同一
Adapter 中依次尝试 Channel 1、16，前一端点协议确认失败后才进入下一端点。通道号只用于传输，
...
降噪档位切换已在 FreeBuds Pro 3 实机验证通过；家族 Adapter 不开放档位。FreeClip 与 FreeClip 2
为开放式夹耳，无降噪与通透，不发送 `2B 2A` 状态查询，仅以合法电量帧确认协议并开放组件电量。
